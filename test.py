...
async def report(session:ClientSession,cookies:Dict[str,str],date: str, state: str):
    date = date
    state = state
    # cookies = {'.ncov2019selfreport': '07949E51561E425F0E4C83CA7507D5261F25621A46630467D663CC39565D807E2C9914021F9E85E5962FC5A8CD90DEF3A80BC9559557068E65B9C4E182CADC088D5DF8CD660A4EAAD879F97A3727519D1FB738830A4AB00E35F87AF98993062A757570752B04A72ECDA3878BA3D9D6F3'}
    cookies = cookies
    viewstate = getViewState(cookies)
    ...
    async with session.post(url='https://selfreport.shu.edu.cn/XueSFX/HalfdayReport.aspx',headers=headers, params=params, cookies=cookies, data=data) as response:
        html = await response.text()
        res = res_find.search(html)
        if res:
            print(res[1])
